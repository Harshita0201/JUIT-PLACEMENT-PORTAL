...
placementUpdatesRouter.get("/", async (request, response) => {
  try {
    ...
    response.json(
      placementUpdates.map((placementUpdate) => placementUpdate.toJSON())
    );
  } catch (error) {
    console.log(error);
...
placementUpdatesRouter.post("/", async (request, response) => {
  try {
    ...
    const presentCompanyData = await companyData.findOne({
...
placementUpdatesRouter.put("/:id", async (request, response) => {
  try {
    ...
    const updatedPlacementUpdate = await PlacementUpdate.findByIdAndUpdate(
      request.params.id,
      placementUpdate,
      { new: true }
    );
